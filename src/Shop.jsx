...
function SearchBar({ value, onChange }) {
    return (
        <div className="input-group mb-3 d-flex justify-content-center">
            <input type="text" className="form-control" placeholder="Search products..." value={value} onChange={onChange} style={{ maxWidth: "300px" }} />
...
function SearchButtons({ onClick }) {
    return (
        <div className="d-flex justify-content-center">
            <button className="btn btn-outline-secondary me-2" onClick={() => onClick("electronics")}>Electronics</button>
            <button className="btn btn-outline-secondary me-2" onClick={() => onClick("jewelery")}>Jewelery</button>
            <button className="btn btn-outline-secondary me-2" onClick={() => onClick("men's clothing")}>Men's Clothing</button>
            <button className="btn btn-outline-secondary" onClick={() => onClick("women's clothing")}>Women's Clothing</button>
        </div>
    );
}
                ...
                <div className="position-fixed top-50 start-50 translate-middle p-3 bg-white shadow-lg rounded">
                    ...
                    {items.length === 0 ? (
                        <>
                            ...
                            <button className="btn btn-outline-secondary mt-3 ms-3" onClick={toggleCart}>Close Cart</button>
                        </>
                    ) : (
                        <>
                            <ul className="list-group mb-3">
                                {items.map(item => (
                                    <li key={item.id} className="list-group-item d-flex justify-content-between align-items-center">
                                        ...
                                        <button className="btn btn-outline-secondary btn-sm" onClick={() => removeFromCart(item)}>X</button>
                                    </li>
                                ))}
                            </ul>
                        ...
                        <div key={product.id} className="col">
                            <div className="card h-100">
                                ...
                                <div className="card-body">
                                    ...
                                    <button className="btn btn-outline-secondary" onClick={() => addToCart(product)}>Add to my cart</button>
                                </div>
                            </div>
                        </div>
                    ))}
